...
| 1급 목표 | **과부하·리소스 분배.** 사용자 결정(2026-09-15): 자원 분배가 괜찮으면 한 건이 15초를 넘어도 된다. 시간은 기록하되 합격 조건이 아니다 |
| 속도 목표 | 웜 p50 10초 이하(국어 3쪽)는 유지 확인만. 부하 p95 15초는 참고치이며 게이트가 아니다 |
| 정확성 기준값 | **데스크톱 앱 AI 인식 결과를 오라클**로 쓰고, 어긋난 문항만 Fable이 crop 이미지를 보고 판정해 정답을 확정한다 |
| 속도 레버 | 체험판 전용 모듈만 바꾸는 L3·L5·L6은 기본 포함. 공용 파이프라인을 건드리는 L1·L2와 실험 L4는 **부하 점검에서 분배 문제(504, 메모리 초과, 슬롯 대기 초과 다발)가 나오고 원인이 CPU 시간일 때만** |
...
| 축 | 기준선 | 목표 | 재는 방법 |
|---|---|---|---|
| 과부하·분배 | 2건 겹침 12~17초, 20건 미측정 | 동시 20건에서 504와 JSON 아닌 500이 0건, 슬롯 대기 초과는 미차감 `busy`, RSS 2 GB의 60% 이하, 요청이 한 인스턴스에 몰리지 않음. p95는 기록만(참고 15초) | §6 부하 프로브·고장 주입 |
...
- `problem_parser` 모듈 로드 시 `PIL.Image.MAX_IMAGE_PIXELS = 20_000_000`을 설정한다. Pillow는 이 값의 1배에서 경고, 2배에서 오류를 내므로 실제 차단선은 40M px이고 2×A3 200 DPI ≈ 15.5M px는 경고선 아래다(구현 중 정정, 2026-09-16). 스펙 §6에 있었으나 구현되지 않았던 항목이다.
...
| 점검 | 방법 | 기준 |
|---|---|---|
| 동시 요청 분배 | `scripts/trial_bench/load.py`로 프리뷰에 국어 3쪽을 5·10·20건 동시 업로드. 요청별 wall·status·`instance_id`·`timing_ms` 기록. `TRIAL_PARSE_CONCURRENCY` 1과 2를 각각 측정 | 실패 0건이면서 인스턴스당 겹침이 적고 p95가 낮은 값을 운영 설정으로 채택. 1일 때 2번째 요청이 슬롯 대기 대신 새 인스턴스로 가는지 확인 |
...
| L2 crop 메모리 경로 | `_render_problem_asset`이 crop을 엔트리에 보관, `Image.open(crop_path)` 4곳이 메모리 이미지를 우선. PNG 쓰기 생략 | 0.25초 | 분배 문제 + CPU 원인 | Opus |
| L4 표 탐지 스위치 | 텍스트 PDF 체험판에서 `find_tables` 생략 | 0.34초 | 분배 문제 + CPU 원인이고 코퍼스 IoU 무손실 | Sonnet + Haiku |

- 합격: L1·L2·L5·L6은 코퍼스 문항 번호·박스 바이트 동일. L3은 미리보기 픽셀만 변경(응답 크기 3.5 MB 이하 유지). L4는 지표 게이트.
- 부하 점검에서 분배 문제가 없으면 L1·L2·L4는 하지 않는다. 필요해지면 그때 별도 계획으로 연다.
...
| `problem_parser.py` | 수정 | `render`·`segment`·`entries`·`crops` 시간 분리, `inspect_pdf` 복잡도 |
