...
class TestSupabaseRest(unittest.TestCase):
    ...
    def test_http_and_network_errors_become_quota_unavailable(self):
        for failure in (_http_error(500), _http_error(401), urllib.error.URLError("down"), TimeoutError()):
...
class TestSupabaseQuotaStore(unittest.TestCase):
    def test_consume_maps_row_to_decision(self):
        rest = FakeRest([{"allowed": False, "remaining": 0, "reason": "ip"}])
        decision = SupabaseQuotaStore(rest).consume(day=date(2026, 9, 15), subject="abc", limit=3, global_limit=500)
        self.assertEqual(QuotaDecision(allowed=False, remaining=0, reason="ip"), decision)
        self.assertEqual(
            ("rpc", "trial_consume", {"p_day": "2026-09-15", "p_subject": "abc", "p_limit": 3, "p_global_limit": 500}),
            rest.calls[0],
        )
    ...
    def test_consume_rejects_unexpected_shape(self):
        for bad in (None, [], [{}], {"allowed": True}, [{"allowed": True, "remaining": 1}, {"allowed": True, "remaining": 1}]):
            with self.assertRaises(QuotaUnavailable):
                SupabaseQuotaStore(FakeRest(bad)).consume(day=date(2026, 9, 15), subject="a", limit=3, global_limit=5)

    def test_refund_cleanup_event_and_ping(self):
        rest = FakeRest()
        store = SupabaseQuotaStore(rest)
        store.refund(day=date(2026, 9, 15), subject="abc")
        store.cleanup(today=date(2026, 9, 15))
        store.record_event({"kind": "popup", "feature": "edb", "action": "open"})
        self.assertTrue(store.ping())
        self.assertEqual(
            [
                ("rpc", "trial_refund", {"p_day": "2026-09-15", "p_subject": "abc"}),
                ("rpc", "trial_cleanup", {"p_today": "2026-09-15"}),
                ("insert", "trial_events", {"kind": "popup", "feature": "edb", "action": "open"}),
...
class TestMemoryQuotaStore(unittest.TestCase):
    """The in-memory store mirrors trial_consume so server tests exercise real limit semantics."""

    def test_matches_sql_semantics(self):
        store = MemoryQuotaStore()
        day = date(2026, 9, 15)
        results = [store.consume(day=day, subject="a", limit=3, global_limit=4) for _ in range(4)]
        self.assertEqual(
            [
        ...
        self.assertEqual(QuotaDecision(True, 2, None), store.consume(day=day, subject="b", limit=3, global_limit=4))
        self.assertEqual(QuotaDecision(False, 0, "global"), store.consume(day=day, subject="c", limit=3, global_limit=4))
        store.refund(day=day, subject="b")
        self.assertEqual(QuotaDecision(True, 2, None), store.consume(day=day, subject="c", limit=3, global_limit=4))
        store.refund(day=day, subject="nobody")
        store.refund(day=day, subject="nobody")
        self.assertGreaterEqual(min(store.used.values()), 0)

    def test_events_ping_and_unavailable_switch(self):
        store = MemoryQuotaStore()
        store.record_event({"kind": "parse"})
        self.assertEqual([{"kind": "parse"}], store.events)
        self.assertTrue(store.ping())
        store.cleanup(today=date(2026, 9, 15) + timedelta(days=30))
        store.available = False
        self.assertFalse(store.ping())
        with self.assertRaises(QuotaUnavailable):
            store.consume(day=date(2026, 9, 15), subject="a", limit=3, global_limit=5)
        with self.assertRaises(QuotaUnavailable):
            store.record_event({"kind": "parse"})
